...
const { BrowserView, ipcMain: ipc, app, shell, dialog } = require("electron");
...
function destroyView(mainWindow, id) {
  if (!viewMap[id]) {
    return;
  }
...
export function destroyAllViews(mainWindow) {
  for (const id in viewMap) {
    destroyView(mainWindow, id);
  }
}
