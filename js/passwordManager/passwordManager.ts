...
class PasswordManagers {
  ...
  constructor() {
    ...
    webviews.bindIPC("password-autofill", (tab, args, frameId) => {
      ...
      this.getConfiguredPasswordManager().then(async (manager) => {
        ...
        manager
          ...
          .then((credentials) => {
            if (credentials != null) {
              webviews.callAsync(tab, "getURL", (err, topLevelURL) => {
                if (err) {
                  console.warn(err);
                  return;
                }
    ...
    webviews.bindIPC("password-autofill-check", (tab, args, frameId) => {
      webviews.callAsync(tab, "sendToFrame", [
        frameId,
...
export default new PasswordManagers();
