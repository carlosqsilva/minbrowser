...
function getBestInput(names, exclusionNames, types) {
  ...
  for (const field of allFields) {
    ...
    if (
      ...
      checkAttributes(
        field,
        ["name", "formcontrolname", "id", "placholder"],
        names
      )
    ) {
      if (
        !checkAttributes(
          field,
          ["name", "formcontrolname", "id", "placeholder"],
          exclusionNames
        ) &&
        field.type !== "hidden"
      ) {
...
ipc.on("password-autofill-match", (e, data) => {
